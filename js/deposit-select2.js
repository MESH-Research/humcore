 ...
 *
 * Based on:
 *  formatFASTSubjectSelectionAdmin
 * but modified to be used on admin/dashboard screens
 * 
 * @param subject
 * @returns {string}
 */
 function formatFASTSubjectSelectionAdmin(subject) {
    ...
    if (subject.auth) {
		...
		//console.log('formatFASTSubjectSelectionAdmin (subject.auth) formatted_subject = ', formatted_subject); 
    } else {
		//
		// if there is no subject.auth create the display string from subject.text
		const [id, auth, facet] = subject.text.split(":");
		formatted_subject = `${auth} (${facet})`;
		//console.log('formatFASTSubjectSelectionAdmin (subject.text) formatted_subject = ', formatted_subject); 
	}
	return formatted_subject;
}
